...
整体任务:我们自己去定义一个类型，可以装任意数据类型，任意长度的容器
    ...
    v4:提供一些额外的功能
    查询相关的方法：
        4.1提供一个查看容器存储了多少个数据的方法
        4.2提供一个方法根据索引查看里面存储的内容
        4.3查询数据在容器里面第一次出现的位置
        4.4查询数据在容器里面最后一次出现的位置
        4.5查询数据在容器里面出现的次数
    更改数据相关的方法
        4.6根据索引位置替换里面的数据
        4.7根据使用新数据替换数组里面的指定的数据【第一个】
        4.8根据使用新数据替换数组里面的指定的数据【全部】
    删除相关的方法
        4.9根据索引删除对饮位置的数据
        4.10删除指定数据【删除在容器里面出现的第一个】
        4.11删除指定数据【删除在容器里面全部指定数据】
...
public class MyArrayList {
    ...
    public Object getByIndex(int index){
        ...
        if (index<0||index>=size){
            throw new ArrayIndexOutOfBoundsException("参数不合法，请提供0-"+(size-1)+"的数字");
        }
        return arr[index];
    }
    ...
    //4.7根据使用新数据替换数组里面的指定的数据【第一个】
    public void replaceFirst(Object o,Object e){
        for (int i = 0; i < arr.length; i++) {
            if (o.equals(arr[i])){
                //如果找到了一个相等，直接替换，不用再去找下一个
               arr[i]=e;
               return;
            }
        }
    ...
    //4.8根据使用新数据替换数组里面的指定的数据【全部】
    public void replaceAll(Object o,Object e){
        for (int i = 0; i < arr.length; i++) {
            if (o.equals(arr[i])){
                //遍历所有，如果找到了相等，直接替换
                arr[i]=e;
    ...
    public void remove(int index){
        //对index参数进行校验
        if(index<0||index>=size){
            throw new ArrayIndexOutOfBoundsException("参数index不合法,请提供0-"+(size-1)+"之间的数字");
        }
        //删除数据
